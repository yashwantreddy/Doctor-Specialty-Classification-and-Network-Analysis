...
for i,proc in enumerate(merged.procedure):
    ...
    if 'typically' in w[0].split():
        indices.append(i)
merged_no_cards = merged[merged.specialty!='Cardiology']
...
no_card_words=[]
for line in merged_no_cards.procedure:
    no_card_words.append(line.replace('_', ' '))
    
d = path.dirname(__file__)
doc_mask = np.array(Image.open(path.join(d, "hrt.jpg")))
wc = WordCloud(font_path='C:\Windows\Fonts\FREESCPT.TTF',width = 1200, height = 750,scale=5, background_color="white", max_words=2000, mask=doc_mask,colormap='hot')
wc.generate(' '.join(card_words))
wc.to_file(path.join(d, "heart_cloud.png"))
